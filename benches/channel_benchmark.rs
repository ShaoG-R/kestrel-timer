            ...
            |b, &size| {
                ...
                b.to_async(&runtime).iter_custom(|iters| async move {
                    ...
                    for _ in 0..iters {
                        ...
                        // Send all
                        for ch in &channels {
                            ch.0.notify(TaskCompletion::Called);
                        }
...
fn bench_rtrb_spsc_creation_comparison(c: &mut Criterion) {
    let mut group = c.benchmark_group("rtrb_spsc_creation_comparison");
    
    const CAPACITY: usize = 100;
 
    // Custom rtrb-based ring buffer
    group.bench_function("custom_rtrb_ring_buffer", |b| {
        b.iter(|| {
            let (_producer, _consumer) = rtrb::RingBuffer::<u32>::new(CAPACITY);
        });
    });
...
fn bench_bounded_creation_comparison(c: &mut Criterion) {
    ...
    group.bench_function("custom_rtrb_spsc", |b| {
        b.iter(|| {
            let (_tx, _rx) = spsc::channel::<u32>(CAPACITY);
        });
    });
