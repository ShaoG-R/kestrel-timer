...
use kestrel_timer::utils::ringbuf;
...
fn benchmark_creation(c: &mut Criterion) {
    ...
    for capacity in [4, 8, 16, 32, 64, 128, 256] {
        ...
        group.bench_with_input(
            ...
            |b, &cap| {
                b.iter(|| {
                    let (producer, consumer) = ringbuf::new::<u64>(black_box(cap));
...
fn benchmark_single_thread_throughput(c: &mut Criterion) {
    ...
    for capacity in [8, 32, 128] {
        ...
        group.bench_with_input(
            ...
            |b, &cap| {
                b.iter(|| {
                    let (mut producer, mut consumer) = ringbuf::new::<u64>(cap);
...
fn benchmark_batch_operations(c: &mut Criterion) {
    ...
    group.bench_function("custom_smallvec_batch", |b| {
        b.iter(|| {
            let (mut producer, mut consumer) = ringbuf::new::<u64>(capacity);
    ...
    group.bench_function("custom_smallvec_create_destroy_32", |b| {
        b.iter(|| {
            for _ in 0..iterations {
                let (mut producer, mut consumer) = ringbuf::new::<u64>(black_box(capacity));
...
fn benchmark_push_only(c: &mut Criterion) {
    ...
    group.bench_function("custom_smallvec_push", |b| {
        b.iter(|| {
            let (mut producer, _consumer) = ringbuf::new::<u64>(capacity);
...
fn benchmark_pop_only(c: &mut Criterion) {
    ...
    group.bench_function("custom_smallvec_pop", |b| {
        b.iter_batched(
            || {
                let (mut producer, consumer) = ringbuf::new::<u64>(capacity);
                for i in 0..items {
