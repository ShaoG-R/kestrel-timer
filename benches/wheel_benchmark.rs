            ...
            b.to_async(&runtime).iter_custom(|iters| async move {
                ...
                for _ in 0..iters {
                    ...
                    let tasks: Vec<TimerTask> = (0..size)
                        .map(|i| {
                            TimerTask::new_oneshot(Duration::from_millis(100 + i as u64 * 10), None)
                        })
                        .collect();
        ...
        b.to_async(&runtime).iter_custom(|iters| async move {
            ...
            for _ in 0..iters {
                ...
                let tasks: Vec<TimerTask> = (0..100)
                    .map(|i| {
                        TimerTask::new_oneshot(Duration::from_millis(20 + (i % 20) * 10), None)
                    })
                    .collect();
                let _result = timer.register_batch(handles, tasks);
            ...
            for _ in 0..iters {
                ...
                for round in 0..10 {
                    ...
                    let tasks: Vec<TimerTask> = (0..50)
                        .map(|_| {
                            TimerTask::new_oneshot(Duration::from_millis(50 + round * 10), None)
                        })
                        .collect();
                    let _result = timer.register_batch(handles, tasks);
