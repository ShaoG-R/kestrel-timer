...
enum ServiceCommand {
    ...
    /// 
    /// 添加批量定时器句柄，仅包含必要数�? task_ids �?completion_rxs
    AddBatchHandle {
        task_ids: Vec<TaskId>,
    ...
    /// 
    /// 添加单个定时器句柄，仅包含必要数�? task_id �?completion_rx
    AddTimerHandle {
        task_id: TaskId,
...
///     
///     // Register one-shot tasks (注册一次性任�?
...
///     
///     // Register periodic tasks (注册周期性任�?
...
impl TimerService {
    ...
    ///     println!("Timer fired!"); // 定时器触�?
    /// }));
    /// let task = TimerTask::new_oneshot(Duration::from_secs(10), callback);
    /// service.register(handle, task).unwrap(); // 注册定时�?
    /// 
    /// // Cancel task
    ...
    ///     .map(|i| {
    ///         let callback = Some(CallbackWrapper::new(move || async move {
    ///             println!("Timer {} fired!", i); // 定时器触�?
    ///         }));
    ///         TimerTask::new_oneshot(Duration::from_secs(10), callback)
    ///     })
    ///     .collect();
    /// service.register_batch(handles, tasks).unwrap(); // 注册定时�?
    /// 
    /// // Batch cancel
    ...
    /// - 任务 ID 在延期后保持不变
    /// - 原始超时通知保持有效
    /// - 如果 callback �?`Some`，将替换原始回调
    /// - 如果 callback �?`None`，保留原始回�?
    ///
    /// # Examples (示例)
    /// ```no_run
    /// # use kestrel_timer::{TimerWheel, TimerService, CallbackWrapper, TimerTask, config::ServiceConfig};
    ...
    /// service.register(handle, task).unwrap(); // 注册定时�?
    /// 
    /// // Postpone and replace callback (延期并替换回�?
...
/// service.register_batch(handles, tasks).unwrap();
    ...
    /// 注册定时器任务到服务 (注册阶段)
    /// # 参数
    /// - `handle`: 通过 `allocate_handle()` 分配�?handle
    /// - `task`: 通过 `TimerTask::new_oneshot()` �?`TimerTask::new_periodic()` 创建的任�?
    ///
    /// # 返回�?
    /// - `Ok(TimerHandle)`: 注册成功
    /// - `Err(TimerError::RegisterFailed)`: 注册失败 (内部通道已满或关�?
    ///
    /// # Examples (示例)
    /// ```no_run
    ...
    /// ```
    #[inline]
    pub fn register(&self, handle: crate::task::TaskHandle, task: crate::task::TimerTask) -> Result<TimerHandle, TimerError> {
        let task_id = handle.task_id();
    ...
    /// - `Ok(BatchHandle)`: 注册成功
    /// - `Err(TimerError::RegisterFailed)`: 注册失败 (内部通道已满或关�?
    /// - `Err(TimerError::BatchLengthMismatch)`: handles �?tasks 长度不匹�?
    ///
    /// # Examples (示例)
    /// ```no_run
    /// # use kestrel_timer::{TimerWheel, TimerService, CallbackWrapper, config::ServiceConfig, TimerTask};
...
impl ServiceActor {
    ...
    async fn run(self) {
        ...
        // 一次性任�?futures：每�?future 返回 (TaskId, TaskCompletion)
        ...
        // 周期性任�?futures：每�?future 返回 (TaskId, Option<PeriodicTaskCompletion>, mpsc::Receiver)
        // 返回接收器以便我们可以继续监听下一个事�?
        ...
        // �?shutdown_rx �?self 中移出，以便�?select! 中使�?&mut
        ...
        loop {
            tokio::select! {
                ...
                Some((task_id, completion)) = oneshot_futures.next() => {
                    ...
                    // 检查完成原因，只转�?Called 事件，不转发 Cancelled 事件
                ...
                Some((task_id, reason, mut receiver)) = periodic_futures.next() => {
                    ...
                    // 如果�?Cancelled �?None，不重新添加 future（任务结束）
                }
...
mod tests {
    ...
    async fn test_task_timeout_cleans_up_task_sender() {
        ...
        // Wait a moment to ensure internal cleanup is complete (等待片刻以确保内部清理完�?
