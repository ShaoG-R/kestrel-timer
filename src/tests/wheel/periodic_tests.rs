...
use crate::task::{CallbackWrapper, CompletionReceiver, TaskCompletion, TimerTask, TimerTaskWithCompletionNotifier};
...
fn test_periodic_task_basic() {
    ...
    assert!(wheel.task_index.get(task_id.key()).is_some(), "Task should still be in index");
...
fn test_periodic_task_cross_layer() {
    ...
    assert!(wheel.task_index.get(task_id.key()).is_some(), "Task should be reinserted");
    let location = wheel.task_index.get(task_id.key()).unwrap();
...
fn test_mixed_oneshot_and_periodic_tasks() {
    ...
    for id in &oneshot_ids {
        assert!(wheel.task_index.get(id.key()).is_none(), "Oneshot task should be removed");
    }
    
    // Verify all periodic tasks are still present
    // 验证所有周期性任务仍然存在
    for id in &periodic_ids {
        assert!(wheel.task_index.get(id.key()).is_some(), "Periodic task should still be present");
    }
